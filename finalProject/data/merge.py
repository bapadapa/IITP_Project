...
drop_col = [
    # 실질적 일반 사람에게 피해가 없는 파라미터 제거
    '직무유기','직권남용','장물','수뢰','증뢰','실화','명예','권리행사 방해','낙태','위증과 증거인멸','무고','손괴','간통','도박과 복표',
    '폭력행위등(단체등의구성 활동)''혼인빙자간음','통화','유가증권인지우표','비밀침해','신앙','문서','인장','교통방해','기타음란행위',
    '유가증권·인지·우표','권리행사방해','신용업무경매','기타 음란행위','신용업무 경매','사기','횡령','배임','범죄소계',
    '폭력행위등(손괴강요주거침입등)','폭력행위 등 처벌에 관한법률','과실치사상','업무상 과실치사상',
    # 특정 대학의 수는 의미가 없다 판단하여 총계를 제외한 대학수 제거
    '일반대 수','전문대 수','교육대 수','산업대 수',
    # 성비를 이용한 인구수는 기타 데이터와 연계시 데이터 부족에 의하여 제거
    '한국인(남자인구수[명])','한국인(여자인구수[명])',
    # 전입,전출 칼럼을 보유하면 순이동이 필요 없다 판단하여 제거
    '순이동 (명)','시도간전입 (명)','시도간전출 (명)',
    # 시도내 이동 데이터는 불필요하다 판단하여 제거
    '시도내이동-시군구내 (명)','시도내이동-시군구간 전입 (명)','시도내이동-시군구간 전출 (명)',
    ]
